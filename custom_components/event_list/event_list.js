class EventList extends HTMLElement {
  ...
  async render() {
    ...
    try {
      const eventData = await this.fetchData(`${this.baseUrl}/jsonapi/node/event`); // Updated for events
      const events = eventData.data || [];

      // Filter and sort events
      const filteredEvents = events
        .filter(event => {
          const title = event.attributes.title.toLowerCase();
      ...
      const limitedEvents = this.filterNumber ? filteredEvents.slice(0, this.filterNumber) : filteredEvents;
      ...
      const resultsHtml = await Promise.all(limitedEvents.map(async event => {
        const title = event.attributes.title;
        const eventImageRelationship = event.relationships?.field_event_image?.data; // Assuming a field for event image
        const imageUrl = eventImageRelationship
          ? await this.getImageUrl(eventImageRelationship.id, eventImageRelationship.type)
          : this.svgPlaceholderBase64;

        const pathAlias = event.attributes.path?.alias || '#';
